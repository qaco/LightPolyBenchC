...
sed 's/DATA_TYPE POLYBENCH_2D(\([a-zA-Z]*\),\([A-Z]*\),\([A-Z]*\),[a-z]*,[a-z]*)/DATA_TYPE \1\[\2\]\[\3\]/g' $1 \
    ...
    | sed 's/POLYBENCH_2D_ARRAY_DECL(\([a-zA-Z0-9]*\),[[:space:]]*\([a-zA-Z_]*\),[[:space:]]*\([A-Z]*\),[[:space:]]*\([A-Z]*\),[[:space:]]*[a-z]*,[[:space:]]*[a-z]*)/\2 \1\[\3\]\[\4\]/g' \
    | sed 's/POLYBENCH_3D_ARRAY_DECL(\([a-zA-Z0-9]*\),[[:space:]]*\([A-Z_]*\),[[:space:]]*\([A-Z]*\),[[:space:]]*\([A-Z]*\),[[:space:]]*\([A-Z]*\),[[:space:]]*[a-z]*,[[:space:]]*[a-z]*,[[:space:]]*[a-z]*)/\2 \1\[\3\]\[\4\]\[\5\]/g' \
    | sed 's/POLYBENCH_1D_ARRAY_DECL(\([_a-zA-Z0-9]*\),[[:space:]]*\([a-zA-Z_]*\),[[:space:]]*\([A-Z]*\),[[:space:]]*[a-z]*)/\2 \1\[\3\]/g' \
